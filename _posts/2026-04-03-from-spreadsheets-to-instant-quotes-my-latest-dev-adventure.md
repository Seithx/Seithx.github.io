...
*   For state persistence, I added `localStorage` to keep the form state persistent, so the app remembers where you left off even after a restart.
*   **Custom icon:** Built a custom app icon for the Windows EXE build, giving it a nice polished feel.
*   Dealing with ESLint v9 dropping mid-refactor completely broke my existing config. That took way too long to sort out.
*   And the usual Windows GitBash `npm` output weirdness, resorting to redirecting output to temp files. A recurring theme on my setup!
