...
These were the biggest hurdles and what I learned.
...
*   `yt-dlp` can be a memory hog. When pulling from a large channel playlist, it tried to load the entire list of videos into memory before outputting anything, causing it to hang. The flags `--lazy-playlist` and `--flat-playlist` processed the playlist item by item, turning a firehose into a manageable stream.

*   Pipes are fragile. My script kept dying with "Broken pipe" or "Invalid argument" errors. The culprit was my Python script consuming `stdin` for itself, leaving the piped data from `yt-dlp` with nowhere to go. The fix was to pass file paths as arguments to the Python script (`python -c '...' "$OUT_FILE"`) instead of relying on the standard input stream.
